...
const getUsageSummary = async (req, res) => {
  try {
    ...
    usageRecords.forEach((record) => {
      const date = record.date instanceof Date ? record.date.toISOString().split("T")[0] : record.date;
      if (!graphData[date]) {
        graphData[date] = {
          date: date,
          fuelUsedKg: 0,
    ...
    const graph = Object.values(graphData).sort((a, b) =>
...
const getUserStats = async (req, res) => {
  try {
    ...
    // Total fuel, events, and minutes across all usage entries for user's stoves
    ...
    const totalFuelUsed = parseFloat(
      totalsAgg[0]?.getDataValue("total_fuel") || 0
    );
    const totalCookingEvents = parseInt(
      totalsAgg[0]?.getDataValue("total_events") || 0,
      10
    );
    const totalCookingTime = parseInt(
      totalsAgg[0]?.getDataValue("total_minutes") || 0,
      10
    );
    ...
    const fuelChartData = dateList.map(
      (d) => Math.round((fuelByDate[d] || 0) * 100) / 100
    );
    ...
    const sumMinutes = parseInt(
      minutesAgg[0]?.getDataValue("sum_minutes") || 0,
      10
    );
    const sumEvents = parseInt(
      minutesAgg[0]?.getDataValue("sum_events") || 0,
      10
    );
    res.json({
      totalStoves,
...
const getStoveUsage = async (req, res) => {
  try {
    ...
    if (!stove) {
      return res
        .status(404)
        .json({ error: "Stove not found or not owned by user" });
    }
